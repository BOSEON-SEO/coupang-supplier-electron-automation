    ...
    <div className="cal-shell" style={{flexDirection:'row'}}>
      ...
      <div className="cal-sidebar">
        <div style={{display:'flex', alignItems:'center', gap:8, padding:'4px 8px 14px'}}>
          <button className="btn ghost sm" onClick={onBack}><I.ChevronL size={13}/></button>
          <div style={{fontSize:11, color:'#A1A1AA'}}>달력으로</div>
        </div>
        ...
        <div style={{padding: '0 8px 8px'}}>
          ...
          {isOrphanView && (
            <button className="btn accent" style={{width:'100%', justifyContent:'center'}} onClick={onCreateJob}>
              <I.Plus size={13}/> 새 차수 만들기
            </button>
          )}
          {isAllView && (
      ...
      <div className="cal-shell">
        <div className="cal-header">
          ...
          {!isAllView && !isOrphanView && (
            <div className="badge accent" style={{fontSize:11, padding:'4px 10px'}}>
              {dayJobs.find(j => j.id === selectedJobId)?.label} 보기 — 비포함 행은 흐림
            </div>
          )}
          {isOrphanView && (
        ...
        <div style={{padding: '10px 22px', background: 'var(--bg-elev)', borderBottom: '1px solid var(--border)', display:'flex', alignItems:'center', gap:14, fontSize:11, color:'var(--text-3)'}}>
          <span>범례:</span>
          <span style={{display:'inline-flex', alignItems:'center', gap:5}}><span style={{width:10, height:10, background:'var(--accent-soft)', borderLeft:'3px solid var(--accent)'}}/>선택한 차수</span>
