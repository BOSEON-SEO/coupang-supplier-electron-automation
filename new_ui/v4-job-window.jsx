...
function JobViewV4({ job, vendor, plugins, onRequestPluginWindow }) {
  const { useState, useMemo, useEffect } = React;
  const [view, setView] = useState('review');
  const [rows, setRows] = useState(V4_ACTIVE);
  const [shipInbox, setShipInbox] = useState(V4_SHIP);
  const [milkInbox, setMilkInbox] = useState(V4_MILK);
  ...
  const counts = {
    ...
    history: V4_HIST.filter(h => h.jobId === job.id).length,
  };
  ...
  return (
    <div className="job-shell">
      ...
      <div className="job-view">
        ...
        {view === 'confirm' && <ConfirmViewV4 rows={rows} setRows={setRows} onUpload={() => setUploadStage('countdown')}/>}
        {view === 'admin-sync' && <AdminSyncViewV4 onOpenPluginWindow={() => onRequestPluginWindow('tbnws-admin')}/>}
        {view === 'ship' && <InboxViewV4 kind="ship" items={shipInbox} setItems={setShipInbox} invoicePrinter={invoicePrinter} onExclude={askExcludeInbox}/>}
        {view === 'milk' && <InboxViewV4 kind="milk" items={milkInbox} setItems={setMilkInbox} palletOptim={palletOptim} onExclude={askExcludeInbox}/>}
        {view === 'history' && <HistoryViewV4 job={job}/>}

        {uploadStage && <UploadModalV4 stage={uploadStage} onClose={() => { setUploadStage(null); if (uploadStage === 'done') setView('ship'); }} vendor={vendor}/>}
...
function InboxViewV4({ kind, items, setItems, invoicePrinter, palletOptim, onExclude }) {
  const { useState, useMemo } = React;
  ...
  return (
    <div className="job-view">
      ...
      <div className="tool-row">
        ...
        <div style={{flex:1}}/>
        <button className="btn sm danger" disabled={!selected.size} onClick={() => onExclude(kind, [...selected])}>
          <I.X size={12}/> 이번 차수에서 제외 ({selected.size})
        </button>
        ...
        <div className="inbox-detail">
          {selected.size === 0 ? (
            <div className="empty">
              ...
              <div className="sub">좌측에서 같은 센터의 복합키를 골라 lot을 구성합니다.</div>
            </div>
          ) : (
            <BuilderInlineV4 kind={kind} items={items.filter(i => selected.has(i.id))} onClose={() => setSelected(new Set())} invoicePrinter={invoicePrinter} palletOptim={palletOptim}/>
          )}
        </div>
      </div>
    </div>
...
function BuilderInlineV4({ kind, items, onClose, invoicePrinter, palletOptim }) {
  const { useState } = React;
  const isShip = kind === 'ship';
  ...
  return (
    <div className="job-view">
      <div className="summary-row">
        ...
        <div className="stat"><div className="lbl">전체 누적</div><div className="val">{V4_HIST.length}<span className="u">개</span></div></div>
...
function UploadModalV4({ stage, onClose, vendor }) {
  const steps = [
  ...
  return (
    <div className="overlay">
      <div className="modal" style={{width:460}}>
        {stage === 'countdown' ? <CountdownInnerV4/> : (
          <>
            <div className="modal-head">
            ...
            <div className="modal-body">
              ...
              {stage === 'done' && (
                <div style={{marginTop:12, padding:'10px 14px', background:'var(--ok-soft)', borderRadius:5, fontSize:12, color:'var(--ok)', fontWeight:600, display:'flex', alignItems:'center', gap:8}}>
                  <I.CheckCircle size={14}/>4건 → 쉽먼트, 6건 → 밀크런 인박스로 라우팅됨
                </div>
              )}
            </div>
            <div className="modal-foot">
