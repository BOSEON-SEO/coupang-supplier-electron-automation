...
const { PLUGINS } = window.V3;

function PluginManager({ plugins, setPlugins, onClose }) {
...
// Plugin window — full-screen takeover when admin-sync is opened
function PluginWindow({ pos, setPos, onClose, focused, onFocus, zIndex }) {
  return (
    <DraggableWindow
      title="tbnws · 어드민 동기화"
      subtitle={<>플러그인 작업 창<span className="sep">·</span>v1.2.0</>}
      pluginBadge="PLUGIN"
      pos={pos} setPos={setPos} onClose={onClose} focused={focused} onFocus={onFocus} zIndex={zIndex}
      w={760} h={520}
      kind="plugin"
    >
      <div style={{flex:1, padding:'24px', overflow:'auto', background:'oklch(0.99 0.01 320)'}}>
...
window.PluginWindow = PluginWindow;
